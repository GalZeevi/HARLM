...
def csv_to_scores(csv_path):
    ...
    print('GROUP BY')
    df = group_sum
    print(f'op: SUM, rel_score = {df.loc[:, "average_relative_error"].mean()}, std = {df.loc[:, "average_relative_error"].std()}')
    df = group_avg
    print(f'op: AVG, rel_score = {df.loc[:, "average_relative_error"].mean()}, std = {df.loc[:, "average_relative_error"].std()}')
    df = group_cnt
    print(f'op: CNT, rel_score = {df.loc[:, "average_relative_error"].mean()}, std = {df.loc[:, "average_relative_error"].std()}')
